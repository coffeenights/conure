...
func GenerateRoutes(relativePath string, r *gin.Engine, appHandler *AppHandler) {
	applications := r.Group(relativePath)
	{
		...
		applications.GET("/:organizationID/:applicationID/e/:environment/", appHandler.DetailApplication)
		...
		applications.GET("/:organizationID/:applicationID/e/:environment/c/:componentName/status", appHandler.StatusComponent)
	}
}
